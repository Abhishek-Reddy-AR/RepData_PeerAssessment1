...
```
Mean of the the total no of steps taken per each day is `r mean`  
Median of the total no of steps taken per each day is `r median`
...
```
The 5 min interval ,on average across all the days in the datset,containing the max no of steps is `r dfmax$intervals`-`r dfmax$intervals+5`  
...
total<-sum(lv)
```
Total no of rows with NA's are `r total`
```{r,echo=TRUE}
